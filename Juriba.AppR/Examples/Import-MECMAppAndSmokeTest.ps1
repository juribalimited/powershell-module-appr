...
# The module stores session state in $global:appRConnection (set by
# Connect-JuribaAppR). Use it directly rather than going through a wrapper
# so this script works against module versions that don't expose a
# Get-JuribaAppRSession cmdlet.
$existingSession = $global:appRConnection
...
if ($SkipImport) {
    ...
    # Resolve-AppRId normalises across schema variants — see comment near
    # the top of the script. Drop empty results so $beforeIds only contains
    # real ids and the post-import diff stays correct.
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($new.Count -gt 0) {
            ...
            Write-Host "  ✓ Found new AppR app: id=$pickedId, name=$($apprApp.name)" -ForegroundColor Green
            if ($new.Count -gt 1) {
                $others = $new |
                    Where-Object { (Resolve-AppRId $_) -ne $pickedId } |
                    ForEach-Object { "$(Resolve-AppRId $_):$($_.name)" }
